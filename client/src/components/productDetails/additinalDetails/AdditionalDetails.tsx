...
function AdditionalDetails({ rating }: AdditionalDetailsProps) {
  // State to keep track of the currently active tab (element)
  const [activeElement, setActiveElement] = useState(0);

  const safeRating: Rating = rating ?? { rate: 0, count: 0 };
  ...
  const elements = [
    ...
    {
      title: `Reviews (${safeRating.count})`,
      element: <ReviewList rating={safeRating} />,
    },
  ];
